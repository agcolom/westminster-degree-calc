...
### Analytics
- **Google Analytics 4**: Comprehensive usage tracking to understand student engagement
- **Sequence Tracking**: Monitor how students explore different grade scenarios (debounced to prevent spam)
- **Save/Load Events**: Track feature adoption and returning user behavior
- **Custom Event Parameters**:
  - `calculator_type` (undergraduate/postgraduate)
  - `award_type` (honours, masters, pgcert, etc.)
  - `classification` (Pass, Merit, Distinction, etc.)
  - `calculation_trigger` (button_click vs auto_calculation)
  - `load_status` (success/no_data/error)
...
### Making Updates and Deploying

#### Development Workflow

1. **Make your changes** in the relevant files:
   - Undergraduate calculator: `app/page.tsx`
   - Postgraduate calculator: `app/postgraduate/page.tsx`
   - Layout/Analytics: `app/layout.tsx`
...
After testing and committing your changes to main, deploy to GitHub Pages.
...
```bash
npm run export
cd out && git init && git add -A && git commit -m "Deploy: Description of updates" && git push -f https://github.com/agcolom/westminster-degree-calc.git HEAD:gh-pages
cd ..
```

This creates the `out/` directory with all files and deploys directly to GitHub Pages.

##### Manual Deployment (Alternative)

If you prefer to review files before deployment:
...
Visit https://agcolom.github.io/westminster-degree-calc/ - changes typically appear within 1-2 minutes.

#### Common Tasks

**Update calculator logic:**
- Edit the calculation functions in the respective page files
- Test thoroughly with various input combinations
- Verify edge cases (insufficient credits, boundary classifications)

**Update UI/styling:**
- Modify Tailwind classes directly in components
- For global styles, edit `app/globals.css`
- UI components are in `components/ui/` (from shadcn/ui)

**Update About section:**
- Edit the Accordion content in the page files
- Remember to use British English spellings
- Escape quotes in JSX with `&quot;` and apostrophes with `&apos;`

**Update standalone files:**
- Standalone files are automatically regenerated by `npm run export`
- The `process-html.js` script inlines all CSS, JS, and fonts
- Both UG and PG versions are created simultaneously

**View analytics data:**
- Real-time: Reports → Realtime in GA4
- Historical: Reports → Engagement → Events
- Advanced analysis: Use Explorations to create custom reports
- Register custom dimensions in Admin → Custom definitions for better reporting
...
#### calculate_degree
Fired when a student calculates their classification (either by clicking Calculate or auto-calculation after slider adjustment).

**Parameters:**
- `calculator_type`: undergraduate | postgraduate
- `award_type`: honours, masters, pgcert, pgdip, mres, mfa, erasmus, integrated
- `classification`: Pass, Merit, Distinction, or UG classification names
- `calculation_trigger`: button_click | auto_calculation (debounced 2 seconds for auto)

**Use cases:**
- Track which classifications students are achieving
- Identify most common degree types calculated
- Monitor if students are using sliders to explore "what if" scenarios

#### save_marks
Fired when a student clicks "Save My Marks".
...
- Identify which calculator has more returning users

#### load_marks
Fired when a student clicks "Load My Marks".
...
**Use cases:**
- Track returning users (successful loads)
- Identify users trying to load without saving (no_data)
- Monitor technical issues (errors)
