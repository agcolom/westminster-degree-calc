...
1. **Build the production version** and generate standalone files:
   ```bash
   npm run export
   ```
   ...
   - `ugStandalone.html`
   - `pgStandalone.html`
   - `.nojekyll` file (required for GitHub Pages to serve `_next` directory)

2. **Switch to gh-pages branch** and copy the built files:
   ```bash
   git checkout gh-pages
   cp -r out/* .
   ```

3. **Stage and commit** the deployment files:
   ```bash
...
6. **Verify deployment**:
   - Visit https://agcolom.github.io/westminster-degree-calc/
